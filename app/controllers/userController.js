...
let forgotPassword=(req,res)=>{
    ...
    var token = crypto.randomBytes(20).toString('hex');
        ...
        resetPassword=(req,res)=>{
            UserModel.findOne({ resetPasswordToken: req.params.token, resetPasswordExpires: { $gt: Date.now() } }, function(err, userDetails) {
                if (!userDetails) {
                  res.send({error:'Password reset token is invalid or has expired.'});
                }else{
    
                userDetails.password = bcrypt.hashSync(req.body.password, 10);
